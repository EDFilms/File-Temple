...
void jo_write_mpeg(FILE *fp, const unsigned char *rgb, int width, int height, int fps) {
	...
	for (int vblock=0; vblock<(height+15)/16; vblock++) {
		for (int hblock=0; hblock<(width+15)/16; hblock++) {
			...
			for (int i=0; i<256; ++i) {
				...
				const unsigned char *c = rgb + y*width*3+x*3;
